...
if __name__ == '__main__':
    ...
    parser.add_argument('-conf_path', default="conf_synth/correxp.conf")
    ...
    for i in range(n_runs):
        ...
        recall, closest_rule_score_value, closest_unification_score_value = train_model(kb, templates, conf, relationships=symbol_relationships, test_kb=test_kb)
        rule_scores.append(closest_rule_score_value)
        unification_scores.append(closest_unification_score_value)

        print(relationships)

    print(conf["model"], conf["experiment"])

    correlation = np.corrcoef(rule_scores, unification_scores)
    with summary_writer.as_default(), tf.contrib.summary.always_record_summaries():        
        tf.contrib.summary.scalar("correlation", correlation, step=0)        
